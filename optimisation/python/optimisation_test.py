...
class Test_Optimisation_Algorithms(unittest.TestCase):
    def test_greedy_welfare_calculation(self):
        ...
        for _ in range(100):
            ...
            w_unreliable_list.append(w_un_avg)
            w_reliable_list.append(w_rel_avg)
            # within_bound.append(1) if self.assertLessEqual(abs(w_un_avg - w2), w_un_avg*0.01) else within_bound.append(0)

        print("Unreliable Welfare Stats:")
        print("Mean:", sum(w_unreliable_list)/250)
        print("STD:", statistics.stdev(w_unreliable_list))

        print("Reliable Welfare Stats:")
        print("Mean:", sum(w_reliable_list)/250)
        print("STD:", statistics.stdev(w_reliable_list))
